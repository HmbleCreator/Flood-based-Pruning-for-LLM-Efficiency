...
This paper is driven by a single question: \emph{Can an attention head be
causally indispensable even when its weight magnitude is among the smallest
in the network?} Magnitude-based pruning answers this question in the
negative by construction. We show that the answer is yes---and that a
simple, gradient-free observational metric recovers these heads,
challenging a core assumption of neural-network pruning.
...
build upon. The head thus plays a structurally diffuse role: it appears
low-importance by weight magnitude, yet is foundational for downstream
computation.
