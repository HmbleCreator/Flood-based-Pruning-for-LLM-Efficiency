...
\end{tikzpicture}
\caption{Conceptual pipeline of our research program: causal perturbation interventions are used to estimate the routing dependency graph, which is analyzed via RIE centrality descriptors, validated through OLS damage regressions, and exploited for FLOOD pruning.}
\label{fig:pipeline}
\end{figure}
...
\caption{Directed routing dependency network $G$ estimated for GPT-2 Small, grouped by layers. Layer-grouped self-attention heads show directed representational dependencies $D(u,v)$ extracted via exhaustive causal perturbation.}
...
\caption{Sorted eigenvalues of the graph Laplacian for the GPT-2 Small routing graph. The second smallest eigenvalue defines the algebraic connectivity (Fiedler value $\lambda_2$), indicating the network's routing redundancy.}
\label{fig:spectral}
\end{figure}
...
\begin{enumerate}
    \item \textbf{Broadcast Centrality ($f_{\text{bcast}}$):} Measures the extent to which a head acts as a global information source. Computed via PageRank on the transpose dependency matrix (reverse PageRank) \citep{page1999pagerank} as defined in Equation~\ref{eq:bcast}:
    \begin{equation}
        f_{\text{bcast}}(u) \;=\; d \sum_{v \ne u} \frac{D(v, u)}{\text{out-degree}(v)} f_{\text{bcast}}(v) + \frac{1-d}{N}
        \label{eq:bcast}
    \end{equation}
    \item \textbf{Receiver Centrality ($f_{\text{recv}}$):} Measures the extent to which a head acts as a global information sink. Computed via forward PageRank as defined in Equation~\ref{eq:recv}:
    \begin{equation}
        f_{\text{recv}}(u) \;=\; d \sum_{v \ne u} \frac{D(u, v)}{\text{in-degree}(v)} f_{\text{recv}}(v) + \frac{1-d}{N}
        \label{eq:recv}
    \end{equation}
    \item \textbf{Injector Centrality ($f_{\text{inj}}$):} Measures local feedforward influence to the immediate next layer, representing direct downstream injection as defined in Equation~\ref{eq:inj}:
    \begin{equation}
...
\end{tikzpicture}
\caption{Standardized OLS regression coefficients ($\beta$) with 95\% bootstrap confidence intervals (error bars) across three model families. Broadcast centrality is consistently the strongest source-like predictor of causal damage.}
...
\begin{subfigure}[b]{0.32\textwidth}
    ...
    \caption{Pythia-160M}
\end{subfigure}
\caption{Standardized causal damage vs. Broadcast Centrality (reverse PageRank) across attention heads. The OLS fit demonstrates a strong, highly statistically significant relationship across architectures, validating that global routing descriptors predict causal nécessité.}
...
\caption{Language modeling perplexity vs. pruning budget on WikiText-2 for GPT-2 Small. FLOOD methods closely track the oracle Gradient-Taylor curve while significantly outperforming the Magnitude baseline.}
...
\caption{Language modeling perplexity recovery curves during fine-tuning on WikiText-2 for GPT-2 Small at a 30\% pruning budget. FLOOD-pruned models converge rapidly and stably back to baseline performance compared to magnitude-based pruning.}
\label{fig:recovery}
\end{figure}
...
\caption{Downstream representational subspace alignment and routing hub overlap preservation under FLOOD vs. Magnitude pruning at a 30\% budget for GPT-2 Small, showing that FLOOD preserves the model's communication backbone.}
\label{fig:topology}
\end{figure}
...
These observations motivate future work in which the weighting coefficients of the RIE family are dynamically parameterized based on global graph statistics as shown in Equation~\ref{eq:adaptive_rie}:
\begin{equation}
