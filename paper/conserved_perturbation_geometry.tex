...
\begin{enumerate}
    ...
    \item \textbf{Experiment C: Multi-Layer Simultaneous Projection.} We project out $u_1^{(l)}$ at \emph{all layers 0--4 simultaneously} and sweep $\alpha \in [0.1, 1.0]$. As shown in Table~\ref{tab:multi_layer_projection}, the cumulative effect grows monotonically with $\alpha$, reaching $1.20\times$ at $\alpha = 1.0$ (Wilcoxon $p = 0.015$), confirming that the multi-layer intervention compounds across layers.
