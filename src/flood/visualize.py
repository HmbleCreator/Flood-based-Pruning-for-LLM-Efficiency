...
def plot_perplexity_curves(results, safe_name):
    ...
    Plots perplexity curves comparing:
      FLOOD vs. FLOOD-Simplified vs. Magnitude vs. Attention Entropy vs. Random vs. Bridge-Only vs. Gradient-Taylor
    """
    ...
    if "FLOOD-Simplified" in results:
        plt.plot(budgets, results["FLOOD-Simplified"], 'v-.', color='maroon', linewidth=2.0, markersize=6, label="FLOOD (Simplified: Bridge+Broadcast)")
    plt.plot(budgets, results["Bridge-Only"], 'd-.', color='darkorange', linewidth=2.0, markersize=6, label="Bridge-Only (Paper 1)")
    if "Broadcast-Only" in results:
        plt.plot(budgets, results["Broadcast-Only"], 'h-.', color='forestgreen', linewidth=2.0, markersize=6, label="Broadcast-Only (Paper 3)")
    if "Gradient-Taylor" in results:
        plt.plot(budgets, results["Gradient-Taylor"], '*--', color='purple', linewidth=1.5, markersize=5, label="Gradient Importance (Taylor)")
...
def plot_recovery_curves(results, safe_name):
    """Plots perplexity recovery curves across fine-tuning updates."""
    steps = results["Recovery_Steps"]
    plt.figure(figsize=(7.5, 6))
    
    plt.plot(steps, results["Recovery_FLOOD"], 'o-', color='crimson', linewidth=2.5, markersize=7, label="FLOOD (Full Routing)")
    if "Recovery_FLOOD-Simplified" in results:
        plt.plot(steps, results["Recovery_FLOOD-Simplified"], 'v-.', color='maroon', linewidth=2.0, markersize=6, label="FLOOD (Simplified: Bridge+Broadcast)")
    plt.plot(steps, results["Recovery_Magnitude"], 's--', color='darkblue', linewidth=1.8, markersize=5, label="Magnitude")
    if "Recovery_Gradient-Taylor" in results:
        plt.plot(steps, results["Recovery_Gradient-Taylor"], '*--', color='purple', linewidth=1.8, markersize=5, label="Gradient-Taylor")
    plt.plot(steps, results["Recovery_Random"], 'x--', color='gray', linewidth=1.8, markersize=5, label="Random")
